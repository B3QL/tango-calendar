...
class TestEventsView(TestCase):
    ...
    def test_location_tenants(self):
        ...
        other_tenant_user = UserFactory.create(company_id="029cf390-4234-494d-b464-0000deadbeef")
    ...
    def test_events_timezones(self):
        ...
        create_response = self.client.post(self.events_url, data={
    ...
    def test_events_location_filtering(self):
        ...
        response = self.client.get(self.events_url, data={'location_id': with_location.location.id})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 1)

    def test_events_search(self):
        _event_with_name = MeetingFactory.create(event_name="A", meeting_agenda="B", location=self.location)
        _event_with_agenda = MeetingFactory.create(meeting_agenda="A", event_name="B", location=self.location)
        _filtered_event = MeetingFactory.create(event_name="B", meeting_agenda="B", location=self.location)
    ...
    def test_events_length_limit(self):
        ...
        self.assertEqual(response.json(), {'non_field_errors': ['Meetings shouldn’t be longer than 8 hours.']})

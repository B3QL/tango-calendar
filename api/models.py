...
class Meeting(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name="meetings", editable=False)
    event_name = models.TextField()
    meeting_agenda = models.TextField()
    start = models.DateTimeField()
    end = models.DateTimeField()
    participant_list = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name="participate")
